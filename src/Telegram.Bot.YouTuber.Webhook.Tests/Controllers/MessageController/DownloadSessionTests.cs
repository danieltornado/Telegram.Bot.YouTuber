...
public sealed class DownloadSessionTests
{
    ...
    public async Task WhenReceivedAudioSelection_Update_ShouldDownloadSession()
    {
        ...
        Mock<IStickService> stickServiceMock = new();
    ...
    /// <summary>
    /// When application receives a third message with an audio selection, but there isn't available space 
    /// </summary>
    /// <returns></returns>
    [Fact]
    public async Task WhenReceivedAudioSelectionAndThereIsNotAvailableSpace_Update_ShouldDownloadSession()
    {
        // -------ARRANGE-------
        long chatId = 123;
        int messageId = 456;
        ...
        // error has been sent
        telegramServiceMock
            .Verify(e => e.SendMessageAsync(chatId, messageId, It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Once);

        // downloading has never been executed
        downloadingClientMock
            .Verify(e => e.DownloadAsync(It.IsAny<SessionContext>(), It.IsAny<SessionMediaContext>(), It.IsAny<SessionMediaContext>(), It.IsAny<CancellationToken>()), Times.Never);
